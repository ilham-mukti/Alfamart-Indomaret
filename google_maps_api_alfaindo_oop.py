...
class GooglePlaces:
	def __init__(self, path, keyword, list_kelurahan, kecamatan, kota, api_key, type=1):
		...
		self.count = 1
		self.path = path+self.keyword+"/"+self.kecamatan+"/"
		self.type = type
	...
	def start(self):
		if(self.type==1):
			...
			for self.kelurahan in self.list_kelurahan:
				datas = pd.read_csv(self.path+'type_1_'+self.keyword+'_kelurahan_'+self.kelurahan+'.csv')
				datas_sorted = datas.sort_values('distance', ascending=False)
				self.datas = datas_sorted.head(2)
	...
	def extract_lat_lng(self):
		...
		self.save_to(just_folder=True)
		df_kelurahan.to_csv(self.path+f"lat_long_kecamatan_{self.kecamatan}.csv", index=False)
		return df_kelurahan
	...
	def parse_places(self, params, page=0):
		...
		for data in result['results']:
			...
			if(self.type!=1):
				...
				origin = (self.latitude, self.longitude)  # (latitude, longitude) don't confuse
				dist = (lat, lng)
	...
	def save_to(self, dict_row=False, just_folder=False):
		if(just_folder):
			if not os.path.isdir(self.path):
				os.makedirs(self.path)
		else:
			if(self.type==2):
				path_output = self.path+ "2/type_"+ str(self.type) + "_"+ self.keyword + "_kelurahan_" +self.kelurahan+".csv"
			else:
				path_output = self.path+ "type_"+ str(self.type) + "_"+self.keyword+"_kelurahan_" +self.nama_kelurahan+".csv"
